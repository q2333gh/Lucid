...
/* Forward declaration */
static idl_status coerce_impl(idl_arena *arena, const idl_type_env *env,
...
static idl_status coerce_impl(idl_arena *arena, const idl_type_env *env,
  ...
  if (et->kind == IDL_KIND_OPT) {
    ...
    if (inner_et && !idl_type_is_optional_like(env, inner_et)) {
      ...
      idl_value *coerced_inner;
      idl_status st =
          coerce_impl(arena, env, wt, et->data.inner, value, &coerced_inner);
      if (st == IDL_STATUS_OK) {
        *out = idl_value_opt_some(arena, coerced_inner);
        return *out ? IDL_STATUS_OK : IDL_STATUS_ERR_ALLOC;
      }
    ...
    for (size_t i = 0; i < expected_count; i++) {
      ...
      for (size_t j = 0; j < value->data.record.len; j++) {
        if (value->data.record.fields[j].label.id == ef->label.id) {
          ...
          if (wire_field_type) {
            idl_status st = coerce_impl(arena, env, wire_field_type, ef->type,
...
idl_status idl_skip_value(const uint8_t *data, size_t data_len, size_t *pos,
  ...
  case IDL_KIND_OPT: {
    ...
    if (flag == 1) {
      ...
      idl_status st = idl_skip_value(data, data_len, pos, env, wt->data.inner,
                                     &inner_skipped);
      if (st != IDL_STATUS_OK)
        return st;
    }
