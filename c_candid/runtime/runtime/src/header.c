...
static idl_status read_uleb128(const uint8_t *data, size_t data_len,
...
static idl_status read_sleb128(const uint8_t *data, size_t data_len,
...
static const char *make_table_var(idl_arena *arena, int64_t index) {
  char buf[32];
  snprintf(buf, sizeof(buf), "table%ld", (long)index);
...
static idl_status index_to_type(idl_arena *arena, int64_t index,
                                uint64_t table_len, idl_type **out) {
  if (index >= 0) {
...
static idl_status parse_cons_type(const uint8_t *data, size_t data_len,
                                  size_t *pos, idl_arena *arena,
                                  uint64_t table_len, idl_type **out) {
  int64_t opcode;
