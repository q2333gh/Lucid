...
static idl_status grow_array(idl_arena *arena, void **arr, size_t *cap,
                             size_t elem_size) {
...
static int find_type_index(const idl_type_table_builder *builder,
                           const idl_type *type, int32_t *out) {
...
static idl_status write_sleb128(idl_arena *arena, uint8_t **buf, size_t *len,
                                size_t *cap, int64_t value) {
...
static idl_status write_uleb128(idl_arena *arena, uint8_t **buf, size_t *len,
                                size_t *cap, uint64_t value) {
  uint8_t tmp[MAX_LEB128_SIZE];
  size_t written;
...
static idl_status encode_type_ref(const idl_type_table_builder *builder,
                                  idl_arena *arena, uint8_t **buf, size_t *len,
                                  size_t *cap, const idl_type *type) {
  int32_t index;
...
idl_status idl_type_table_builder_build_type(idl_type_table_builder *builder,
  ...
  case IDL_KIND_VARIANT: {
    ...
    int64_t opcode = (actual_type->kind == IDL_KIND_RECORD) ? IDL_TYPE_RECORD
                                                            : IDL_TYPE_VARIANT;
