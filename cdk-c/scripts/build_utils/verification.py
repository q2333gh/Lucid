...
def verify_raw_init_import(wasm_file: Path, wasi_sdk_compiler_root: Optional[Path] = None) -> bool:
    ...
    if wasi_sdk_compiler_root:
        wasm_objdump = wasi_sdk_compiler_root / "bin" / "wasm-objdump"
        if not wasm_objdump.exists():
    ...
    try:
        ...
        result = subprocess.run(
            [str(wasm_objdump), "-x", str(wasm_file)],
            capture_output=True,
            text=True,
            check=True
        )
        ...
        print(f"  ⚠️  Could not verify raw_init import (wasm-objdump not available: {e})")
