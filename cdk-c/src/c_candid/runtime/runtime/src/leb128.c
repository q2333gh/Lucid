...
idl_status idl_uleb128_encode(uint64_t value, uint8_t *out, size_t out_len,
                              size_t *written) {
  if (out == NULL) {
...
idl_status idl_sleb128_encode(int64_t value, uint8_t *out, size_t out_len,
                              size_t *written) {
  if (out == NULL) {
    return IDL_STATUS_ERR_INVALID_ARG;
  }
...
idl_status idl_uleb128_decode(const uint8_t *input, size_t input_len,
...
idl_status idl_sleb128_decode(const uint8_t *input, size_t input_len,
                              size_t *consumed, int64_t *value) {
  if (input == NULL || value == NULL) {
    return IDL_STATUS_ERR_INVALID_ARG;
  }
  idl_i128 result = 0;
