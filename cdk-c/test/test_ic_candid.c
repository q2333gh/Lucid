...
Test(ic_candid, sleb128_decodes_negative_numbers_and_errors_on_truncation)
{
    ...
    size_t offset = 0;
    int64_t decoded = 0;
    cr_expect_eq(candid_read_sleb128(encoded_negative, sizeof(encoded_negative), &offset, &decoded), IC_OK);
    cr_expect_eq(decoded, -123456);
    cr_expect_eq(offset, sizeof(encoded_negative));

    const uint8_t truncated[] = {0x80};
    offset = 0;
    cr_expect_eq(candid_read_sleb128(truncated, sizeof(truncated), &offset, &decoded), IC_ERR_INVALID_ARG);
...
Test(ic_candid, serialize_and_deserialize_text)
{
    ...
    cr_expect_eq(candid_deserialize_text(ic_buffer_data(&buf), ic_buffer_size(&buf), &offset, &decoded, &decoded_len), IC_OK);
    ...
    offset = 0;
    decoded = NULL;
    cr_expect_eq(candid_deserialize_text(wrong_tag, sizeof(wrong_tag), &offset, &decoded, &decoded_len), IC_ERR_INVALID_ARG);
}

Test(ic_candid, serialize_and_deserialize_nat_and_int)
{
    ic_buffer_t buf = make_buffer();
    const uint64_t nat_value = 9'007'199'254'740'991ULL;
    cr_expect_eq(candid_serialize_nat(&buf, nat_value), IC_OK);

    size_t offset = 0;
    uint64_t decoded_nat = 0;
    cr_expect_eq(candid_deserialize_nat(ic_buffer_data(&buf), ic_buffer_size(&buf), &offset, &decoded_nat), IC_OK);
    ...
    cr_expect_eq(candid_deserialize_int(ic_buffer_data(&buf), ic_buffer_size(&buf), &offset, &decoded_int), IC_OK);
...
Test(ic_candid, serialize_and_deserialize_blob_and_principal)
{
    ...
    size_t decoded_len = 0;
    cr_expect_eq(candid_deserialize_blob(ic_buffer_data(&buf), ic_buffer_size(&buf), &offset, &decoded_blob, &decoded_len), IC_OK);
    ...
    cr_expect_eq(ic_principal_from_bytes(&principal, principal_bytes, sizeof(principal_bytes)), IC_OK);

    buf = make_buffer();
    cr_expect_eq(candid_serialize_principal(&buf, &principal), IC_OK);
    offset = 0;
    ic_principal_t decoded_principal = {0};
    cr_expect_eq(candid_deserialize_principal(ic_buffer_data(&buf), ic_buffer_size(&buf), &offset, &decoded_principal), IC_OK);
