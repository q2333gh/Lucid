...
#ifndef LLM_C_CANISTER_BUILD
static int parse_vocab_json(GPT2Tokenizer *tok, const char *path) {
    size_t len = 0;
    char  *data = read_file(path, &len);
    if (!data)
        return 0;
    int result = parse_vocab_json_data(tok, data);
    free(data);
    return result;
}
#endif
...
#ifndef LLM_C_CANISTER_BUILD
static int parse_merges(GPT2Tokenizer *tok, const char *path) {
    size_t len = 0;
    char  *data = read_file(path, &len);
    if (!data)
        return 0;
    int result = parse_merges_data(tok, data);
    free(data);
    return result;
}
#endif
...
GPT2Tokenizer *tokenizer_load(const char *vocab_path, const char *merges_path) {
    ...
    if (!tok)
        return NULL;
    if (!parse_vocab_json(tok, vocab_path)) {
        tokenizer_free(tok);
        return NULL;
    }
    if (!parse_merges(tok, merges_path)) {
        tokenizer_free(tok);
        return NULL;
    }
