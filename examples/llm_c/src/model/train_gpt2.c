...
#ifdef LLM_C_CANISTER_BUILD
#include "ic_c_sdk.h"
#define GPT2_PRINTF(...) ((void)0)
#define GPT2_EXIT(code) ic_api_trap("gpt2 failure")
#else
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#define GPT2_PRINTF printf
#define GPT2_EXIT exit
#endif
        ...
        for (int t = 0; t < T; t++) {
            for (int h = 0; h < NH; h++) {
                ...
                for (int t2 = 0; t2 <= t; t2++) {
                    for (int t3 = 0; t3 <= t; t3++) {
                        ...
                        float local_derivative =
                            att_bth[t2] * (indicator - att_bth[t3]);
                ...
                for (int t2 = 0; t2 <= t; t2++) {
                    float *key_t2 = inp + b * T * C3 + t2 * C3 + h * hs +
                                    C; // +C because it's key
                    float *dkey_t2 = dinp + b * T * C3 + t2 * C3 + h * hs +
                                     C; // +C because it's key
                    for (int i = 0; i < hs; i++) {
                        // in the forward pass this was:
...
static size_t gpt2_prepare_param_layout(GPT2 *model, const int *model_header) {
    ...
    GPT2_PRINTF("[GPT-2]\n");
    GPT2_PRINTF("max_seq_len: %d\n", maxT);
    GPT2_PRINTF("vocab_size: %d\n", V);
    GPT2_PRINTF("num_layers: %d\n", L);
    GPT2_PRINTF("num_heads: %d\n", NH);
    GPT2_PRINTF("channels: %d\n", C);
    ...
    GPT2_PRINTF("num_parameters: %zu\n", num_parameters);
    model->num_parameters = num_parameters;
    return num_parameters;
}

/*
 * The stdio-based checkpoint loader is unused in canister builds that define
 * LLM_C_CANISTER_BUILD, so exclude the implementation to avoid importing file
 * IO helpers from wasi_snapshot_preview1.
 */
#ifndef LLM_C_CANISTER_BUILD
void gpt2_build_from_checkpoint(GPT2 *model, char *checkpoint_path) {

    // read in model from a checkpoint file
    FILE *model_file = fopen(checkpoint_path, "rb");
    if (model_file == NULL) {
        GPT2_PRINTF("Error opening model file\n");
        GPT2_EXIT(1);
    }
    int model_header[256];
    fread(model_header, sizeof(int), 256, model_file);
    if (model_header[0] != 20240326) {
        GPT2_PRINTF("Bad magic model file");
        GPT2_EXIT(1);
    }
    if (model_header[1] != 1) {
        GPT2_PRINTF("Bad version in model file");
        GPT2_EXIT(1);
    }
...
void gpt2_build_from_buffer(GPT2 *model, const uint8_t *buffer, size_t len) {
    if (buffer == NULL) {
        GPT2_PRINTF("Error: checkpoint buffer is null\n");
        GPT2_EXIT(1);
    }
    size_t header_bytes = sizeof(int) * 256;
    if (len < header_bytes) {
        GPT2_PRINTF("Error: checkpoint buffer is too small\n");
        GPT2_EXIT(1);
    }
    int model_header[256];
    memcpy(model_header, buffer, header_bytes);
    if (model_header[0] != 20240326) {
        GPT2_PRINTF("Bad magic model buffer");
        GPT2_EXIT(1);
    }
    if (model_header[1] != 1) {
        GPT2_PRINTF("Bad version in model buffer");
        GPT2_EXIT(1);
    }
    size_t num_parameters = gpt2_prepare_param_layout(model, model_header);
    size_t params_bytes = num_parameters * sizeof(float);
    if (len < header_bytes + params_bytes) {
        GPT2_PRINTF("Error: checkpoint buffer truncated\n");
        GPT2_EXIT(1);
    }
    model->params_memory =
        malloc_and_point_parameters(&model->params, model->param_sizes);
...
void gpt2_forward(GPT2 *model, int *inputs, int *targets, int B, int T) {
    ...
    if (model->params_memory == NULL) {
        GPT2_PRINTF("Error: model was not initialized properly.\n");
        GPT2_EXIT(1);
    }
    ...
    if (model->acts_memory == NULL) {
        ...
        GPT2_PRINTF("num_activations: %zu\n", num_activations);
        ...
        model->targets =
            (int *)malloc(B * T * sizeof(int)); // might be unused if we never
                                                // have targets but it's small
    } else {
        // validate B,T is no larger than what was previously allocated
        // in principle, we could re-allocate a larger chunk of memory, for now
        // we just error out
        if (B > model->batch_size || T > model->seq_len) {
            GPT2_PRINTF(
                "Error: batch size or sequence length is inadequately large\n");
            GPT2_PRINTF("Model: B=%d T=%d, Desired: B=%d T=%d\n",
...
void gpt2_zero_grad(GPT2 *model) {
    ...
    if (model->grads_acts_memory != NULL) {
        memset(model->grads_acts_memory, 0,
               model->num_activations * sizeof(float));
    }
}
...
void gpt2_backward(GPT2 *model) {
    ...
    if (model->mean_loss == -1.0f) {
        GPT2_PRINTF("Error: must forward with targets before backward\n");
        GPT2_EXIT(1);
    }
    ...
    // we kick off the chain by filling in dlosses with 1.0f/(B*T), to get the
    // mean loss
    ...
    matmul_backward(grads_acts.lnf, grads.wte, NULL, grads_acts.logits,
    ...
    for (int l = L - 1; l >= 0; l--) {
        ...
        matmul_backward(dl_fch_gelu, dl_fcprojw, dl_fcprojb, dl_fcproj,
        ...
        matmul_backward(dl_atty, dl_attprojw, dl_attprojb, dl_attproj, l_atty,
                        l_attprojw, B, T, C, C);
        attention_backward(dl_qkv, dl_preatt, dl_att, dl_atty, l_qkv, l_att, B,
                           T, C, NH);
        matmul_backward(dl_ln1, dl_qkvw, dl_qkvb, dl_qkv, l_ln1, l_qkvw, B, T,
                        C, 3 * C);
...
void dataloader_init(DataLoader *loader, char *filename, int B, int T) {
    ...
    if (loader->tokens_file == NULL) {
        GPT2_PRINTF("Error opening tokens file\n");
        GPT2_EXIT(1);
    }
...
int main() {
    ...
    GPT2_PRINTF("train dataset num_batches: %d\n", train_loader.num_batches);
    DataLoader val_loader;
    dataloader_init(&val_loader, val_tokens, B, T);
    GPT2_PRINTF("val dataset num_batches: %d\n", val_loader.num_batches);
    ...
    for (int step = 0; step <= 40; step++) {
        ...
        if (step > 0 && step % 20 == 0) {
            ...
            for (int t = 1; t < gen_max_length; t++) {
                ...
                // leaving this alone because you want separate code for
                // inference anyway the inference here is just for sanity
                // checking purposes
                gpt2_forward(&model, gen_tokens, NULL, 1, t);
                float *probs =
                    model.acts.probs + (t - 1) * model.config.vocab_size;
                float coin = random_f32(&rng_state);
                int   next_token =
                    sample_mult(probs, model.config.vocab_size, coin);
                gen_tokens[t] = next_token;
            }
            GPT2_PRINTF("generated: ");
            for (int t = 0; t < gen_max_length; t++) {
                GPT2_PRINTF("%d ", gen_tokens[t]);
            }
            GPT2_PRINTF("\n");
        }
        ...
        GPT2_PRINTF("step %d: train loss %f (took %f ms)\n", step,
                    model.mean_loss, time_elapsed_s * 1000);
    }
