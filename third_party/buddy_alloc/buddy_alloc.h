...
struct buddy *
buddy_init(unsigned char *at, unsigned char *main, size_t memory_size);

/* Initializes a binary buddy memory allocator at the specified location using a
 * non-default alignment */
struct buddy *buddy_init_alignment(unsigned char *at,
                                   unsigned char *main,
                                   size_t         memory_size,
                                   size_t         alignment);
 ...
 * The arena's capacity is reduced to account for the allocator metadata.
 */
struct buddy *buddy_embed_alignment(unsigned char *main,
                                    size_t         memory_size,
                                    size_t         alignment);
 ...
 * Use to get a new handle to the allocator when the arena is moved or copied.
 */
struct buddy *buddy_get_embed_at_alignment(unsigned char *main,
                                           size_t         memory_size,
                                           size_t         alignment);
...
void *
buddy_calloc(struct buddy *buddy, size_t members_count, size_t member_size);

/* Realloc semantics are a joke. See realloc. */
void *buddy_realloc(struct buddy *buddy,
                    void         *ptr,
                    size_t        requested_size,
                    bool          ignore_data);

/* Realloc-like behavior that checks for overflow. See reallocarray */
void *buddy_reallocarray(struct buddy *buddy,
                         void         *ptr,
 ...
 * span. */
enum buddy_safe_free_status
buddy_safe_free(struct buddy *buddy, void *ptr, size_t requested_size);
...
void buddy_unsafe_release_range(struct buddy *buddy,
                                void         *ptr,
                                size_t        requested_size);
 ...
 *
 * This will store a header at the start of the arena that contains the function
 * pointer (tracker) and a void* (context). The tracker will be called with the
 * context, the start of changed memory and its length.
 *
 * This function MUST be called before any allocations are performed!
 *
 * Change tracking is in effect only for allocation functions, resizing
 * functions are excluded from it.
 *
 * This is an experimental feature designed to facilitate integration with
 * https://github.com/spaskalev/libpvl
 *
 * The API is not (yet) part of the allocator contract and its semantic
 * versioning!
 */
void buddy_enable_change_tracking(struct buddy *buddy,
                                  void         *context,
                                  void (*tracker)(void *,
                                                  unsigned char *,
                                                  size_t));
...
to_buddy_tree_interval(struct buddy_tree *t, struct buddy_tree_pos pos);
...
static enum buddy_tree_release_status
buddy_tree_release(struct buddy_tree *t, struct buddy_tree_pos pos);
...
/* Implementation defined */
static void buddy_tree_debug(struct buddy_tree    *t,
                             struct buddy_tree_pos pos,
                             size_t                start_size);
...
static inline struct bitset_range to_bitset_range(size_t from_pos,
                                                  size_t to_pos);

static void bitset_set_range(unsigned char *bitset, struct bitset_range range);

static void bitset_clear_range(unsigned char      *bitset,
                               struct bitset_range range);

static size_t bitset_count_range(unsigned char      *bitset,
                                 struct bitset_range range);
...
static void bitset_shift_left(unsigned char *bitset,
                              size_t         from_pos,
                              size_t         to_pos,
                              size_t         by);

static void bitset_shift_right(unsigned char *bitset,
                               size_t         from_pos,
                               size_t         to_pos,
                               size_t         by);
...
static void buddy_toggle_range_reservation(struct buddy *buddy,
                                           void         *ptr,
                                           size_t        requested_size,
                                           unsigned int  state);
...
struct buddy *buddy_get_embed_at_alignment(unsigned char *main,
                                           ...
                                           size_t         alignment) {
    struct buddy_embed_check check_result =
        buddy_embed_offset(memory_size, alignment);
    if (!check_result.can_fit) {
        return NULL;
    }
...
void *buddy_realloc(struct buddy *buddy,
    ...
    if (!ignore_data) {
        ...
        memmove(destination, source,
                size_for_depth(buddy, current_depth > target_depth
                                          ? current_depth
                                          : target_depth));
    }
...
void buddy_reserve_range(struct buddy *buddy,
                         void         *ptr,
                         size_t        requested_size) {
...
void *buddy_walk(
    ...
    do {
        ...
        if (pos_status != (tree_order - state.current_pos.depth +
        ...
        if (buddy_tree_valid(tree, test_pos) &&
            buddy_tree_status(tree, test_pos)) {
            continue;
        }
...
void buddy_enable_change_tracking(struct buddy *buddy,
    ...
    struct buddy_change_tracker *header =
        (struct buddy_change_tracker *)buddy_main(buddy);

    /* Allocate memory for the change tracking header */
    buddy_reserve_range(buddy, buddy_main(buddy),
                        sizeof(struct buddy_change_tracker));
...
static struct buddy_tree_pos deepest_position_for_offset(struct buddy *buddy,
    ...
    struct buddy_tree_pos pos =
        buddy_tree_leftmost_child(buddy_tree_for(buddy));
    pos.index += index;
    return pos;
}
...
static void buddy_toggle_range_reservation(struct buddy *buddy,
    ...
    main = buddy_main(buddy);
    if ((dst < main) ||
        ((dst + requested_size) > (main + buddy->memory_size))) {
        return;
    }
...
static bool buddy_is_free(struct buddy *buddy, size_t from) {
    ...
    while (buddy_tree_valid(tree, pos) && (pos.index < query_range.to.index)) {
        struct buddy_tree_interval current_test_range =
            to_buddy_tree_interval(tree, pos);
...
void buddy_debug(struct buddy *buddy) {
    ...
    buddy_tree_debug(buddy_tree_for(buddy), buddy_tree_root(),
...
static void update_parent_chain(struct buddy_tree       *t,
                                struct buddy_tree_pos    pos,
...
static inline size_t buddy_tree_size_for_order(struct buddy_tree *t,
                                               uint8_t            to);
static void          write_to_internal_position(struct buddy_tree       *t,
                                                struct internal_position pos,
                                                size_t                   value);
static size_t        read_from_internal_position(unsigned char           *bitset,
                                                 struct internal_position pos);
static inline unsigned char compare_with_internal_position(
    unsigned char *bitset, struct internal_position pos, size_t value);

#ifdef BUDDY_EXPERIMENTAL_CHANGE_TRACKING
static inline void buddy_tree_track_change(struct buddy_tree *t,
                                           unsigned char     *addr,
                                           size_t             length);
#endif /* BUDDY_EXPERIMENTAL_CHANGE_TRACKING */
...
static void buddy_tree_grow(struct buddy_tree *t, uint8_t desired_order) {
    ...
    while (desired_order > t->order) {
        ...
        /* Update the root */
        pos = buddy_tree_right_child(buddy_tree_root());
        update_parent_chain(t, pos, buddy_tree_internal_position_tree(t, pos),
                            0);
    }
}
    ...
    while (desired_order < t->order) {
        ...
        while (buddy_tree_valid(t, left_start)) {
            ...
            bitset_shift_left(buddy_tree_bits(t),
                              current_internal.bitset_location /* from here */,
                              current_internal.bitset_location +
                                  (current_internal.local_offset * node_count /
                                   2) /* up to here */,
                              current_internal.bitset_location -
...
static void buddy_tree_populate_size_for_order(struct buddy_tree *t) {
    ...
    for (size_t i = 0; i <= t->order; i++) {
        *((size_t *)(((unsigned char *)t) + sizeof(*t)) +
          t->size_for_order_offset + i) = size_for_order(t->order, (uint8_t)i);
    }
}
...
static void write_to_internal_position(struct buddy_tree       *t,
    ...
    struct bitset_range clear_range = to_bitset_range(
        pos.bitset_location, pos.bitset_location + pos.local_offset - 1);

    bitset_clear_range(bitset, clear_range);
    if (value) {
        bitset_set_range(bitset,
                         to_bitset_range(pos.bitset_location,
                                         pos.bitset_location + value - 1));
    }

#ifdef BUDDY_EXPERIMENTAL_CHANGE_TRACKING
    /* Ignore the same bucket condition - we don't care if we track one more
     * byte here */
    buddy_tree_track_change(
        t, bitset, clear_range.to_bucket - clear_range.from_bucket + 1);
#endif
}
...
static size_t read_from_internal_position(unsigned char           *bitset,
    ...
    return bitset_count_range(
        bitset, to_bitset_range(pos.bitset_location,
    ...
    do {
        if (state->going_up) {
            if (state->current_pos.index == state->starting_pos.index) {
                ...
                state->current_pos =
                    buddy_tree_parent(state->current_pos); /* Ascend */
            } else {
                state->current_pos = buddy_tree_right_adjacent(
                    state->current_pos); /* Descend right */
                state->going_up = 0;
            }
        } else if (buddy_tree_valid(
                       t, buddy_tree_left_child(state->current_pos))) {
            /* Descend left */
...
static struct buddy_tree_pos buddy_tree_find_free(struct buddy_tree *t,
    ...
    while (current_depth != target_depth) {
        ...
        if (compare_with_internal_position(
                tree_bits, left_internal,
                target_status + 1)) { /* left branch is busy, pick right */
            current_pos = right_pos;
        } else if (compare_with_internal_position(
                       tree_bits, right_internal,
                       target_status +
                           1)) { /* right branch is busy, pick left */
            current_pos = left_pos;
        } else {
            /* One of the child nodes must be read in order to compare it to its
             * sibling. */
            right_status =
                read_from_internal_position(tree_bits, right_internal);
            if (right_status) {
                if (compare_with_internal_position(tree_bits, left_internal,
                                                   right_status)) {
                    current_pos = left_pos; /* Left is equal or more busy than
                                               right, prefer left */
                } else {
                    current_pos = right_pos;
                }
...
static bool buddy_tree_is_free(struct buddy_tree    *t,
                               struct buddy_tree_pos pos) {
    ...
    while (buddy_tree_valid(t, pos)) {
        struct internal_position internal =
            buddy_tree_internal_position_tree(t, pos);
        size_t value =
            read_from_internal_position(buddy_tree_bits(t), internal);
        if (value) {
...
static void buddy_tree_debug(struct buddy_tree    *t,
    ...
    do {
        ...
        BUDDY_PRINTF(
            ...
            "                                                               ");
        BUDDY_PRINTF("pos index: %zu pos depth: %zu status: %zu bitset-len: "
                     "%zu bitset-at: %zu",
                     state.current_pos.index, state.current_pos.depth,
                     pos_status, pos_internal.local_offset,
                     pos_internal.bitset_location);
...
unsigned int buddy_tree_check_invariant(struct buddy_tree    *t,
    ...
    do {
        ...
        size_t left_child_status =
            buddy_tree_status(t, buddy_tree_left_child(pos));
...
static size_t bitset_count_range(unsigned char      *bitset,
    ...
    if (range.from_bucket == range.to_bucket) {
        return popcount_byte(
            bitset[range.from_bucket] &
            bitset_char_mask[range.from_index][range.to_index]);
    }

    result = popcount_byte(bitset[range.from_bucket] &
                           bitset_char_mask[range.from_index][7]) +
             popcount_byte(bitset[range.to_bucket] &
                           bitset_char_mask[0][range.to_index]);
    while (++range.from_bucket != range.to_bucket) {
        result += popcount_byte(bitset[range.from_bucket]);
    }
    return result;
...
static unsigned char count_trailing_zeroes(size_t val) {
    ...
    static const signed char lookup67[67 + 1] = {
        ...
        -1, 13, 10, 17, 62, 60, 28, 42, 30, 20, 51, 25, 44, 55, 47, 5,  32,
        -1, 38, 14, 22, 11, 58, 18, 53, 63, 9,  61, 27, 29, 50, 43, 46, 31,
        37, 21, 57, 52, 8,  26, 49, 45, 36, 56, 7,  48, 35, 6,  34, 33, -1};
...
static inline size_t integer_square_root(size_t op) {
    ...
    size_t result = 0;
    size_t cursor =
        (SIZE_MAX - (SIZE_MAX >> 1)) >> 1; /* second-to-top bit set */
    while (cursor > op) {
        cursor >>= 2;
    }
